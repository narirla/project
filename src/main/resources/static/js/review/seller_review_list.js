...
// ====== 설정값 ======
const LIST_URL     = '/api/review/paging/seller';
const TOT_CNT_URL  = '/api/review/seller/totCnt';
...
function renderOneItem(item, listNumber) {
  ...
  const imgUrl = item.productImageId
    ...
    : '/img/no-img.png';
  const isNew     = isNewWithinDays(item.rcreate, 3);
  const newMark   = isNew ? '<span class="badge new">NEW</span>' : '';
  const buyerTxt = item.nickname ? `구매자 ${item.nickname}` : '';
      ...
      <div class="review_item_summary">
        ...
        <div class="review-info">
          <div class="reviewFirst">
            ...
            <div class="reviewDate">${escapeHTML(dateStr)} ${newMark}</div>
          </div>
          <div class="reviewSecond">
            ...
            <span class="reviewNickname">${buyerTxt} </span>
        ...
        <div class="review-button">
          <div class="btnSelect">
            <div>
              <button type="button" class="btnReport">신고</button>
            </div>
            <div class="toggle">
