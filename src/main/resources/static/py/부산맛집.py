...
# 전체 페이지 데이터 가져오기
def download_all_pusan_restaurants():
    """부산맛집 API 전체 데이터를 순회하여 CSV 저장"""
    try:
        print("부산맛집 전체 데이터를 다운로드하는 중...")

        service_key = 'CryKKi6HaVVnP0WXU4sIp8dcrZgn2wui0UPEU%2BeivronhsULZ8SFW3qxmqgGmyqgpj59gqzMmd8H%2BhWEzjcvBw%3D%3D'
        result_type = 'json'
        base_url = 'http://apis.data.go.kr/6260000/FoodService/getFoodKr'
        ...
        total_count = res.json()['getFoodKr']['totalCount']
        ...
        csv_path = 'pusan_restaurants_data_all.csv'
...
def main():
    ...
    # 1단계: API에서 데이터 다운로드
    print("\n[1단계] API에서 데이터 다운로드")
    original_csv_path = download_all_pusan_restaurants()
